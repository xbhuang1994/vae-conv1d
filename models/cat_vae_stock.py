...
class CategoricalVAEStock(BaseVAE):
    ...
    def __init__(self,
                 ...
                 alpha: float = 30.,
                 **kwargs) -> None:
        super(CategoricalVAEStock, self).__init__()
        ...
        # 这个3是根据几层卷积后，最后一个维度的值
        self.fc_z = nn.Linear(hidden_dims[-1]*3, self.latent_dim * self.categorical_dim)

        # Build Decoder
        modules = []
        # 这个3，同上
        self.decoder_input = nn.Linear(self.latent_dim * self.categorical_dim, hidden_dims[-1]*3)
    ...
    def encode(self, input: Tensor) -> List[Tensor]:
        ...
        :param input: (Tensor) Input tensor to encoder [B x C x length]
    ...
    def decode(self, z: Tensor) -> Tensor:
        ...
        :return: (Tensor) [B x C x length]
        """
        result = self.decoder_input(z)
        # 这个3同上
        result = result.view(-1, self.last_hidden_dims, 3)
